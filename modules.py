...
class BasicConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1, transpose=False, act_norm=False):
        ...
        self.norm = LayerNorm(out_channels, eps=1e-06, data_format='channels_first')
...
class ConvNeXt_bottle(nn.Module):
    ...
    def __init__(self, dim, drop_path=0.0, layer_scale_init_value=1e-06):
        ...
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones(dim), requires_grad=True) if layer_scale_init_value > 0 else None
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
        self.res_conv = nn.Conv2d(dim, dim, kernel_size=1)
    ...
    def forward(self, x, time_emb=None):
        ...
        x = self.dwconv(x)
        if time_emb is not None:
            x = x + rearrange(time_emb, 'b c -> b c 1 1')
        x = x.permute(0, 2, 3, 1)
        x = self.norm(x)
        x = self.pwconv1(x)
        x = self.act(x)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = LayerNorm(planes, eps=1e-06, data_format='channels_first')
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = LayerNorm(planes, eps=1e-06, data_format='channels_first')
        self.conv3 = nn.Conv2d(planes, planes * self.expansion, kernel_size=1, bias=False)
        self.bn3 = LayerNorm(planes * self.expansion, eps=1e-06, data_format='channels_first')
...
class Learnable_Filter(nn.Module):
    ...
    def __init__(self, n_classes=1):
        ...
        self.bn1 = LayerNorm(64, eps=1e-06, data_format='channels_first')
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = LayerNorm(64, eps=1e-06, data_format='channels_first')
        self.relu = nn.SiLU(True)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(nn.Conv2d(inplanes, planes * block.expansion, kernel_size=1, stride=stride, bias=False), LayerNorm(planes * block.expansion, eps=1e-06, data_format='channels_first'))
        layers = []
